...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        for _ in range(self._num_inner_steps + 1):
            ...
            for i, items in enumerate(parameters.items()):
                key, value = items
                parameters[key] = value - (self._inner_lrs[key] * gradients[i])
    ...
    def load(self, checkpoint_step):
        ...
        if os.path.isfile(target_path):
            ...
            # print(state["meta_parameters"])

            for key, value in state["meta_parameters"].items():
                self._meta_parameters[key] = value.to(DEVICE)
